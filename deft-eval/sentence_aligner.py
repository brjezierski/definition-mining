# coding=utf-8
# Script adapted from https://github.com/cisnlp/simalign
...
class EmbeddingLoader(object):
    def __init__(self, model: str = "bert-base-multilingual-cased", device=torch.device('cpu'), layer: int = 8):
        ...
        if model in TR_Models:
            ...
            self.emb_model = model_class.from_pretrained(
    ...
    def get_embed_list(self, sent_batch: List[List[str]]) -> torch.Tensor:
        if self.emb_model is not None:
            with torch.no_grad():
                ...
                hidden = self.emb_model(
                    **inputs.to(self.device))["hidden_states"]
...
class SentenceAligner(object):
    def __init__(self, model: str = "bert", token_type: str = "bpe", distortion: float = 0.0, matching_methods: str = "mai", device: str = "cpu", layer: int = 8):
        ...
        all_matching_methods = {"a": "inter", "m": "mwmf",
                                "i": "itermax", "f": "fwd", "r": "rev"}
        ...
        self.matching_methods = [all_matching_methods[m]
                                 for m in matching_methods]
        self.device = torch.device(device)

        self.embed_loader = EmbeddingLoader(
            model=self.model, device=self.device, layer=layer)

    @staticmethod
    def get_max_weight_match(sim: np.ndarray) -> np.ndarray:
        if nx is None:
            raise ValueError(
                "networkx must be installed to use match algorithm.")
    ...
    @staticmethod
    def get_alignment_matrix(sim_matrix: np.ndarray, tokens_de, tokens_en) -> Tuple[np.ndarray, np.ndarray]:
        m, n = sim_matrix.shape
        # for pair in zip(tokens_de, sim_matrix.argmax(axis=0)):
        # 	print(pair[0], tokens_en[pair[1]])
    ...
    def apply_distortion(sim_matrix: np.ndarray, ratio: float = 0.5) -> np.ndarray:
        ...
        pos_x = np.array([[y / float(shape[1] - 1)
                         for y in range(shape[1])] for x in range(shape[0])])
        pos_y = np.array([[x / float(shape[0] - 1)
    ...
    def iter_max(sim_matrix: np.ndarray, max_count: int = 2) -> np.ndarray:
        ...
        while count < max_count:
            mask_x = 1.0 - np.tile(inter.sum(1)
                                   [:, np.newaxis], (1, n)).clip(0.0, 1.0)
            mask_y = 1.0 - np.tile(inter.sum(0)
                                   [np.newaxis, :], (m, 1)).clip(0.0, 1.0)
            mask = ((alpha_ratio * mask_x) +
                    (alpha_ratio * mask_y)).clip(0.0, 1.0)
    ...
    def get_word_aligns(self, src_sent: Union[str, List[str]], trg_sent: Union[str, List[str]], use_only_rev=False) -> Dict[str, List]:
        if isinstance(src_sent, str):
            src_sent = src_sent.split()
        if isinstance(trg_sent, str):
            trg_sent = trg_sent.split()
        l1_tokens = [self.embed_loader.tokenizer.tokenize(
            word) for word in src_sent]
        l2_tokens = [self.embed_loader.tokenizer.tokenize(
            word) for word in trg_sent]
        bpe_lists = [[bpe for w in sent for bpe in w]
                     for sent in [l1_tokens, l2_tokens]]
        ...
        vectors = self.embed_loader.get_embed_list(
            [src_sent, trg_sent]).cpu().detach().numpy()
        vectors = [vectors[i, :len(bpe_lists[i])] for i in [0, 1]]

        if self.token_type == "word":
            vectors = self.average_embeds_over_words(
                vectors, [l1_tokens, l2_tokens])

        all_mats = {}
        sim = self.get_similarity(vectors[0], vectors[1])
        sim = self.apply_distortion(sim, self.distortion)

        all_mats["fwd"], all_mats["rev"] = self.get_alignment_matrix(
            sim, bpe_lists[1], bpe_lists[0])
        np.set_printoptions(threshold=sys.maxsize)
        if use_only_rev:
            all_mats["inter"] = all_mats["rev"]
        else:
